...
| 15 | 393.0 | 1 | 393 | - | `Nat.gcd.induction` | ∀ {P : ℕ → ℕ → Prop} (m n : ℕ), (∀ (n : ℕ), P 0 n) → (∀ (m n : ℕ), 0 < m → P (n % m) m → P m n) →... |
| 16 | 332.0 | 2 | 166 | - | `Nat.gcd_comm` | ∀ (m n : ℕ), m.gcd n = n.gcd m |
| 17 | 316.0 | 2 | 158 | - | `Nat.div_lt_self` | ∀ {n k : ℕ}, 0 < n → 1 < k → n / k < n |
| 18 | 294.0 | 14 | 21 | - | `ArithmeticFunction.IsMultiplicative.mul` | ∀ {R : Type u_1} [inst : CommSemiring R] {f g : ArithmeticFunction R},   f.IsMultiplicative → g.I... |
...
| 29 | 210.0 | 5 | 42 | - | `PrimeSpectrum.zeroLocus_vanishingIdeal_eq_closure` | ∀ {R : Type u} [inst : CommSemiring R] (t : Set (PrimeSpectrum R)),   PrimeSpectrum.zeroLocus ↑(P... |
| 30 | 204.0 | 4 | 51 | - | `Nat.primeFactorsList_prime` | ∀ {p : ℕ}, Nat.Prime p → p.primeFactorsList = [p] |
| 31 | 194.0 | 2 | 97 | - | `PrimeSpectrum.subset_zeroLocus_iff_le_vanishingIdeal` | ∀ {R : Type u} [inst : CommSemiring R] (t : Set (PrimeSpectrum R)) (I : Ideal R),   t ⊆ PrimeSpec... |
...
| 42 | 138.0 | 2 | 69 | - | `PrimeSpectrum.gc_set` | ∀ (R : Type u) [inst : CommSemiring R],   GaloisConnection (fun s => PrimeSpectrum.zeroLocus s) f... |
...
| 51 | 101.0 | 1 | 101 | - | `PrimeSpectrum.mem_vanishingIdeal` | ∀ {R : Type u} [inst : CommSemiring R] (t : Set (PrimeSpectrum R)) (f : R),   f ∈ PrimeSpectrum.v... |
| 52 | 99.0 | 3 | 33 | - | `Nat.primeFactorsList_unique` | ∀ {n : ℕ} {l : List ℕ}, l.prod = n → (∀ p ∈ l, Nat.Prime p) → l.Perm n.primeFactorsList |
| 53 | 92.0 | 2 | 46 | - | `Ideal.IsPrime.comap` | ∀ {R : Type u} {S : Type v} {F : Type u_1} [inst : Semiring R] [inst_1 : Semiring S] [inst_2 : Fu... |
| 54 | 91.0 | 1 | 91 | - | `Nat.Prime.pos` | ∀ {p : ℕ}, Nat.Prime p → 0 < p |
| 55 | 91.0 | 1 | 91 | - | `PrimeSpectrum.gc` | ∀ (R : Type u) [inst : CommSemiring R],   GaloisConnection (fun I => PrimeSpectrum.zeroLocus ↑I) ... |
| 56 | 87.0 | 3 | 29 | - | `IsLocalization.isPrime_iff_isPrime_disjoint` | ∀ {R : Type u_1} [inst : CommSemiring R] (M : Submonoid R) (S : Type u_2) [inst_1 : CommSemiring ... |
...
| 60 | 80.0 | 2 | 40 | - | `PrimeSpectrum.basicOpen_eq_zeroLocus_compl` | ∀ {R : Type u} [inst : CommSemiring R] (r : R), ↑(PrimeSpectrum.basicOpen r) = (PrimeSpectrum.zer... |
...
| 66 | 75.0 | 1 | 75 | - | `Ideal.IsPrime.mul_mem_iff_mem_or_mem` | ∀ {α : Type u} [inst : CommSemiring α] {I : Ideal α}, I.IsPrime → ∀ {x y : α}, x * y ∈ I ↔ x ∈ I ... |
| 67 | 74.0 | 2 | 37 | - | `Prime.dvd_of_dvd_pow` | ∀ {M : Type u_1} [inst : CommMonoidWithZero M] {p : M}, Prime p → ∀ {a : M} {n : ℕ}, p ∣ a ^ n → ... |
| 68 | 72.0 | 2 | 36 | - | `Zsqrtd.norm_eq_one_iff_mem_unitary` | ∀ {d : ℤ} {a : ℤ√d}, a.norm = 1 ↔ a ∈ unitary (ℤ√d) |
| 69 | 72.0 | 2 | 36 | - | `IsLocalization.AtPrime.isLocalRing` | ∀ {R : Type u_1} [inst : CommSemiring R] (S : Type u_2) [inst_1 : CommSemiring S] [inst_2 : Algeb... |
...
| 74 | 69.0 | 3 | 23 | - | `ArithmeticFunction.coe_mul_zeta_apply` | ∀ {R : Type u_1} [inst : Semiring R] {f : ArithmeticFunction R} {x : ℕ},   (f * ↑ArithmeticFuncti... |
...
| 80 | 65.0 | 5 | 13 | - | `PrimeSpectrum.localization_specComap_range` | ∀ {R : Type u} (S : Type v) [inst : CommSemiring R] [inst_1 : CommSemiring S] [inst_2 : Algebra R... |
...
| 87 | 62.0 | 2 | 31 | - | `PrimeSpectrum.vanishingIdeal_zeroLocus_eq_radical` | ∀ {R : Type u} [inst : CommSemiring R] (I : Ideal R),   PrimeSpectrum.vanishingIdeal (PrimeSpectr... |
| 88 | 60.0 | 6 | 10 | - | `PrimeSpectrum.isIrreducible_zeroLocus_iff_of_radical` | ∀ {R : Type u} [inst : CommSemiring R] (I : Ideal R),   I.IsRadical → (IsIrreducible (PrimeSpectr... |
...
| 94 | 56.0 | 4 | 14 | - | `PrimeSpectrum.isTopologicalBasis_basic_opens` | ∀ {R : Type u} [inst : CommSemiring R],   TopologicalSpace.IsTopologicalBasis (Set.range fun r =>... |
| 95 | 56.0 | 2 | 28 | - | `IsLocalization.isPrime_of_isPrime_disjoint` | ∀ {R : Type u_1} [inst : CommSemiring R] (M : Submonoid R) (S : Type u_2) [inst_1 : CommSemiring ... |
| 96 | 56.0 | 7 | 8 | - | `ArithmeticFunction.sum_eq_iff_sum_smul_moebius_eq` | ∀ {R : Type u_1} [inst : AddCommGroup R] {f g : ℕ → R},   (∀ n > 0, ∑ i ∈ n.divisors, f i = g n) ... |
| 97 | 55.0 | 5 | 11 | - | `PrimeSpectrum.localization_away_comap_range` | ∀ {R : Type u} [inst : CommSemiring R] (S : Type v) [inst_1 : CommSemiring S] [inst_2 : Algebra R... |
| 98 | 54.0 | 2 | 27 | - | `Nat.Coprime.gcd_left` | ∀ {m n : ℕ} (k : ℕ), m.Coprime n → (k.gcd m).Coprime n |
| 99 | 54.0 | 3 | 18 | - | `EulerProduct.summable_and_hasSum_factoredNumbers_prod_filter_prime_tsum` | ∀ {R : Type u_1} [inst : NormedCommRing R] {f : ℕ → R} [inst_1 : CompleteSpace R],   f 1 = 1 →   ... |
| 100 | 52.0 | 4 | 13 | - | `Nat.Prime.emultiplicity_factorial` | ∀ {p : ℕ}, Nat.Prime p → ∀ {n b : ℕ}, Nat.log p n < b → emultiplicity p n.factorial = ↑(∑ i ∈ Fin... |
